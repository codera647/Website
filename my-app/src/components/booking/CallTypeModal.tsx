...
const OPTIONS = [
  {
    ...
    description:
      "New here? A quick walkthrough of what we do and whether we're a fit.",
  },
  {
    ...
    description:
      "Have a specific project in mind? A deeper session to scope it out.",
  },
];

export default function CallTypeModal() {
  const [open, setOpen] = useState(false);
  const [intent, setIntent] = useState<"stratum" | null>(null);
      ...
      ? [
          {
            ...
            title: "Stratum Audit",
            description:
              "Discuss your AI use case, data readiness and the next steps toward a Stratum Blueprint.",
          },
          {
            ...OPTIONS[1],
            title: "Technical Project Discussion",
            description:
              "Explore an existing AI prototype, product architecture or complex integration requirements.",
          },
        ]
      : OPTIONS;
  ...
  useEffect(() => {
    const onOpen = (event: Event) => {
      setIntent(
        (event as CustomEvent<{ intent?: "stratum" }>).detail?.intent ?? null,
      );
      setOpen(true);
    };
    ...
    <AnimatePresence>
      {open && (
        <motion.div
          ...
          aria-modal="true"
          aria-label={
            intent === "stratum" ? "Book a Stratum Audit" : "Choose a call type"
          }
