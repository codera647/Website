...
type RoleOrJob = Job | Role;

/**
 * Interactive RoleCard with card-hover lift and expandable details on desktop.
 */
export function RoleCard({ role }: { role: RoleOrJob }) {
    const responsibilities = role.responsibilities || [];

    return (
        <Link href={`/careers/${role.slug}`} className="group relative block h-full">
            <div className="card-hover relative z-10 flex h-full flex-col justify-between overflow-hidden rounded-2xl border border-line bg-[#7a7a7c] p-7 pb-16 transition-all duration-500 ease-out md:p-8 md:pb-16 md:group-hover:absolute md:group-hover:inset-x-0 md:group-hover:top-0 md:group-hover:z-20 md:group-hover:h-auto md:group-hover:border-ink md:group-hover:bg-ink md:group-hover:shadow-[0_32px_64px_-24px_rgba(17,17,19,0.45)]">
                <div>
