            ...
            <div className="flex min-h-[460px] max-h-[640px] flex-col justify-between bg-surface p-6 md:p-8">
                ...
                <div className="border-b border-line pb-5">
                    <div className="flex flex-wrap items-center justify-between gap-3">
                        <span className="inline-flex items-center gap-2 rounded-none border border-line bg-[#F5F3E4] px-3 py-1 font-heading text-xs font-medium uppercase tracking-[0.14em] text-ink">
                            ...
                            <div className="flex flex-wrap gap-2">
                                {suggestedQuestions.map((q) => (
                                    <button
                                        ...
                                        type="button"
                                        onClick={() => send(q)}
                                        className="rounded-none border border-line bg-[#F5F3E4] px-4 py-2.5 text-left text-sm text-ink transition-all hover:-translate-y-0.5 hover:border-ink hover:shadow-sm"
                                    >
                                        {q}
                                    </button>
                                ))}
                        ...
                        <AnimatePresence initial={false}>
                                    ...
                                    <div
                                        className={
                                            m.role === "user"
                                                ...
                                                : "max-w-[90%] rounded-none border border-line bg-[#F5F3E4] p-4 text-sm leading-relaxed text-ink shadow-sm"
                                        }
                                    >
                                        {m.role === "user" ? (
                                            <p>{m.text}</p>
                            ...
                            {pending && (
                                <motion.div
                                    ...
                                    className="flex items-center gap-1.5 rounded-none border border-line bg-[#F5F3E4] px-4 py-3"
                                >
                ...
                >
                    <input
                        ...
                        disabled={pending}
                        className="flex-1 rounded-none border border-line bg-[#F5F3E4] px-4 py-3 text-sm text-ink placeholder:text-muted focus:border-ink focus:outline-none transition-colors"
                    />
                    <button
                        type="submit"
